...
void SimpleBLECentral_Init( uint8 task_id )
{
  ...
  HalLedSet( (HAL_LED_1 ), HAL_LED_MODE_ON );
	...
	//LSQ_PrintValue("Print value in decimal format 1 = ", 168, NPI_FORMAT_DEC);
	//LSQ_PrintString("\r\n");
	//LSQ_PrintValue("Print value in hexadecimal format2 = 0x",0x88, NPI_FORMAT_HEX);
	//LSQ_PrintString("\r\n");
...
static void PAStation_Process_Time_Read_Event( gattMsgEvent_t *pMsg )
{
			...
			pMsg->msg.readByTypeRsp.numPairs > 0 )
	{
		PAStationChar_Hdl[BLE_PAMONITOR_CHAR_2] = BUILD_UINT16(pMsg->msg.readByTypeRsp.pDataList[0],
																pMsg->msg.readByTypeRsp.pDataList[1]);
		PAStation_SetDevice_Gas(&(pMsg->msg.readByTypeRsp.pDataList[2]),PAMONITORPROFILE_CHAR2_LEN);
