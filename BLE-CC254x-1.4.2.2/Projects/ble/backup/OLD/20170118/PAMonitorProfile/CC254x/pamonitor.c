...
static uint8 PAMonitorProfileChar4[PAMONITORPROFILE_CHAR4_LEN] = {0x00, 0x00, 0x00, 0x00};
                                        
// PAMonitor Profile Characteristic 4 User Description
static uint8 PAMonitorProfileChar4UserDesp[17] = "Get Gas Monitor ";
...
static uint8 PAMonitorProfileChar6[PAMONITORPROFILE_CHAR6_LEN] = {0x54, 0x30, 0x30, 0x30, 0x30, 0x30, 0x31};   //Device name T000001
...
bStatus_t PAMonitorProfile_SetParameter( uint8 param, uint8 len, void *value )
{
  ...
  switch ( param )
  {
	...
	case PAMONITORPROFILE_CHAR4:
      if ( len == PAMONITORPROFILE_CHAR4_LEN ) 
		{
			VOID memcpy( PAMonitorProfileChar4, value, PAMONITORPROFILE_CHAR4_LEN );
		}
		else
		{
			ret = bleInvalidRange;
...
bStatus_t PAMonitorProfile_GetParameter( uint8 param, void *value )
{
  ...
  switch ( param )
  {
    ...
    case PAMONITORPROFILE_CHAR4:
      VOID memcpy( value, PAMonitorProfileChar4, PAMONITORPROFILE_CHAR4_LEN );
      break;
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
      ...
      case PAMONITORPROFILE_CHAR4_UUID:
	  	*pLen = PAMONITORPROFILE_CHAR4_LEN;
        VOID memcpy( pValue, pAttr->pValue, PAMONITORPROFILE_CHAR4_LEN );
        break;
