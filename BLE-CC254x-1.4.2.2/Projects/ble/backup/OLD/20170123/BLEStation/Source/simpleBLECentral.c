...
#define DEFAULT_UPDATE_MIN_CONN_INTERVAL      1  //400

// Maximum connection interval (units of 1.25ms) if automatic parameter update request is enabled
#define DEFAULT_UPDATE_MAX_CONN_INTERVAL      1  //800
...
static uint8 simpleBLEFindTerminalName ( uint8 *pData, uint8 dataLen , uint8 *devname, uint8 len);
 ...
 */
static char buff_name[PAMONITORPROFILE_CHAR6_LEN];
static uint8 simpleBLECentralEventCB( gapCentralRoleEvent_t *pEvent )
{
	switch ( pEvent->gap.opcode )
	{
		...
		case GAP_DEVICE_INFO_EVENT:
		{
								 ...
								 pEvent->deviceInfo.dataLen ) )
			{
				...
				simpleBLEFindTerminalName(pEvent->deviceInfo.pEvtData,pEvent->deviceInfo.dataLen,buff_name,PAMONITORPROFILE_CHAR6_LEN);
				
				//NPI_PrintString("Device Found:");//????   
        		//NPI_PrintString((uint8 *)bdAddr2Str(pEvent->deviceInfo.addr)); 
				NPI_PrintString_Length(buff_name,PAMONITORPROFILE_CHAR6_LEN);         
        		//NPI_PrintString("\r\n");                
        		NPI_PrintValue("     rssi:-", (uint8)(-( pEvent->deviceInfo.rssi )), 10);//??RSSI  
       	 		NPI_PrintString("dB\r\n");  
			}
		}
		break;
